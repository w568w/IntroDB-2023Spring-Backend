...
use sea_orm::{
    ConnectionTrait, EntityTrait, QuerySelect, QueryTrait, Select, SelectTwo, TransactionTrait,
};
use serde::Serialize;
    ...
    R: EntityTrait,
{
    async fn paged<
        C: ConnectionTrait + TransactionTrait,
        D: Borrow<C> + Send + Sync,
        K: From<(T::Model, Option<R::Model>)> + Serialize,
    >(
        self,
        request: PagingRequest,
        db: D,
...
#[async_trait]
impl<T, R> SelectTwoExt<T, R> for SelectTwo<T, R>
where
    T: EntityTrait,
    R: EntityTrait,
{
